...
默认使用 `ghcr.io/hiper25/tvheadend-client-web:latest`，同时提供 `amd64` 和 `arm64` 镜像。Compose 会自动选择当前设备的 CPU 架构，不需要修改配置。也可以通过 `TVHMON_IMAGE` 换成指定版本，例如：
...
打开项目的 GitHub Releases，根据设备下载同一版本的程序和 `.sha256` 文件：Intel 与 AMD 设备选择 `amd64`，ARM 服务器选择 `arm64`。以下以 `amd64` 为例：

```bash
sha256sum -c tvheadend-manager-1.2.2-debian12-amd64.sha256
chmod +x tvheadend-manager-1.2.2-debian12-amd64
```

运行程序：

```bash
TVHMON_DATA_DIR=./data ./tvheadend-manager-1.2.2-debian12-amd64 --host 0.0.0.0 --port 8088
```
